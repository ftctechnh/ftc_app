...
import org.firstinspires.ftc.teamcode.Utilities.Startup.GoldPosition;
...
// Modified from ACME Robotic's gem detector from Relic Recovery
...
public class SamplingOrderDetector extends DogeCVDetector {
    ...
    private GoldPosition currentOrder = GoldPosition.UNKNOWN;
    private GoldPosition lastOrder    = GoldPosition.UNKNOWN;
    ...
    public Mat process(Mat input) {
        ...
        if(choosenWhiteRect.size() >= 2 && chosenYellowRect != null){
            ...
            if(leftCount == 0){
                currentOrder = GoldPosition.LEFT;
            }

            if(leftCount == 1){
                currentOrder = GoldPosition.CENTER;
            }

            if(leftCount >= 2){
                currentOrder = GoldPosition.RIGHT;
            }
            isFound = true;
            lastOrder = currentOrder;

        }else{
            currentOrder = GoldPosition.UNKNOWN;
            isFound = false;
        }
    ...
    public GoldPosition getCurrentOrder() {
        return currentOrder;
    }

    public GoldPosition getLastOrder() {
        return lastOrder;
    }
}

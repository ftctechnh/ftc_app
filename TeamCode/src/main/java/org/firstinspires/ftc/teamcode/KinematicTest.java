...
public class KinematicTest extends OpMode {
    ...
    private double maxVel = 0, maxAccel = 0;
...
//        }, 0, (long) (timeInterval * 1000));
    ...
    public void loop() {
        ...
        robot.setLeftRight(-gamepad1.left_stick_y, -gamepad1.right_stick_y);
        ...
        leftVel = rotationToMeasurement(ticksToRotation(currLeftPosition - lastLeftPosition)) / (currTime-lastTime);

        rightVel = rotationToMeasurement(ticksToRotation(currRightPosition - lastRightPosition)) / (currTime-lastTime);

        avgVel = (leftVel + rightVel) / 2;

        leftAccel = (leftVel - lastLeftVel) / (currTime-lastTime);

        rightAccel = (rightVel - lastRightVel) / (currTime-lastTime);

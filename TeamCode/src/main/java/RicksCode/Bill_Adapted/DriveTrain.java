...
//      gromit.motorright.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
    ...
    public void mecanumDrive90(double speed, double distance, double robot_orientation) {
        double max;
        double multiplier;
        int right_start;
        int left_start;
        ...
        if (distance > 0) {
            while ((((right_rear.getCurrentPosition()-right_start)  -(left_rear.getCurrentPosition()-left_start))/2 < moveCounts) /* ENCODERS*/ ) {//Should we average all four motors?
                double lfpower = Math.cos(Math.toRadians(drive_direction+45));
                double lrpower = Math.sin(Math.toRadians(drive_direction+45));
                double rfpower = Math.sin(Math.toRadians(drive_direction+45));
                double rrpower = Math.cos(Math.toRadians(drive_direction+45));
                ...
                if (abs(correction) > drive_THRESHOLD) {//If you are off
                    ...
                    rrpower += right_adjustment;
                    rfpower += right_adjustment;
            ...
            while (((left_rear.getCurrentPosition()-left_start) - (right_rear.getCurrentPosition()-right_start)) /2 < abs(moveCounts)){//Should we average all four motors?
                double lfpower = Math.sin(Math.toRadians(drive_direction+45));
                double lrpower = Math.cos(Math.toRadians(drive_direction+45));
                double rfpower = Math.cos(Math.toRadians(drive_direction+45));
                double rrpower = Math.sin(Math.toRadians(drive_direction+45));

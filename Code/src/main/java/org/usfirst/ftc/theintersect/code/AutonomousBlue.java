...
public class AutonomousBlue extends SynchronousOpMode {
	...
	public static void turnRightBackwards(double rotation, double power,
			TelemetryDashboardAndLog telemetry) {
		moveRobotEncoder(-rotation, rotation, -power, power, telemetry);
		moveRobotPower(0, 0);
	}

	public static void turnLeftBackwards(double rotation, double power,
			TelemetryDashboardAndLog telemetry) {
		moveRobotEncoder(rotation, -rotation, power, -power, telemetry);
		moveRobotPower(0, 0);
	}

	public static void moveEncoderBackward(double rotation, double power,
			TelemetryDashboardAndLog telemetry) {
		moveRobotEncoder(-rotation, -rotation, -power, -power, telemetry);

	}

	public static void moveEncoderForward(double rotation, double power,
			TelemetryDashboardAndLog telemetry) {
		moveRobotEncoder(rotation, rotation, power, power, telemetry);
	}
   ...
   /*public static void backwardRightTurn(double rotations, double power) {
	...
	public static void dumpClimbersUltra(TelemetryDashboardAndLog telemetry) {
		double[] positions = { 98, 100, 102.8, 105.6, 108.4, 111.2, 114, 116.8,
				129.6, 129.4, 120.2, 125, 137.8, 133.6, 135.4, 137.2, 142,
				141.5, 147.6, 150.4, 153.2, 156, 160, 165, 167, 170 };
		double ultraVal = ultrasonic.getUltrasonicLevel();
		telemetry.addData("Ultrasonic Level", ultraVal);
		telemetry.update();
		if(ultraVal <= 5) {
		...
		telemetry.clearDashboard();
		telemetry.addData("Ultrasonic Level", ultraVal);
		telemetry.addData("Dumper Position",
				positions[(int) ultraVal - 5] / 180.0);
		telemetry.update();
	...
	public static void moveRobotRotations(double rightRotations,
			double leftRotations, double leftPower, double rightPower,
			TelemetryDashboardAndLog telemetry) {
		resetEncoders();
		backRightWheel.setTargetPosition(
				(int) (rightRotations * Functions.neveRestPPR));
		backLeftWheel.setTargetPosition(
				(int) (leftRotations * Functions.neveRestPPR));
	...
	public static void moveRobotEncoder(double rightRotations,
			double leftRotations, double rightPower, double leftPower,
			TelemetryDashboardAndLog telemetry) {
		resetEncoders();
	...
	//1 in ~ 88
	public static void moveRobotInches(double inches, double power,
			TelemetryDashboardAndLog telemetry) {
		moveRobotRotations(inches / Functions.backWheelCircumfrence,
				inches / Functions.backWheelCircumfrence, power, power,
				telemetry);
	}
	...
	public static void stopAtWhite(double power, long timeout,
			TelemetryDashboardAndLog telemetry) {
		moveRobotPower(-power, -power);
	...
	public static void turnRobotRightDegrees(int degrees, double power,
			int timeout, TelemetryDashboardAndLog telemetry) {
	...
	public static void turnRobotLeftDegrees(int degrees, double power,
			int timeout, TelemetryDashboardAndLog telemetry) {
		int start = gyro.getHeading();
		int heading = start;
		long endtime = System.currentTimeMillis() + (timeout * 1000);

...
public class GyroHandlerF implements System {
    ...
    /**
     * Builds a new instance of {@link GyroHandlerF} from the given robot
     * @param robot the robot to use to build the GyroHandlerF
     * @return      the built gyro handler
     */
    public static GyroHandlerF build(Robot robot) {
        return  new GyroHandlerF(
    ...
    public GyroHandlerF(GyroSensor gyro, HardwareListener listener, RobotLogger logger) {
        this.gyro = gyro;
    ...
    private double calcAngleDiff(double current, double target) {
        ...
        if (Math.abs(diff) > 180) { // if shortest path passes 0
            if (current > 180) //if going counterclockwise
                diff = (diff - 360) - diff;

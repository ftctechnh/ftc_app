 ...
 *
 * @author SSI Robotics/Diamond Blades
 * @version 2015-12-16-00-00
 */
public class DiamondBotHardware extends OpMode

{
    ...
    //--------------------------------------------------------------------------
    //
    // v_motor_left_arm && v_motor_right_arm
    //
    /**
    ...
    //
    /**
     * Manage the aspects of the bucket servo.
     */
    ...
    //
    /**
     * Manage the aspects of the bucket release servo.
     */
...

    {
        ...
        //
        double initial_bucket_position = 0.5;
        double initial_bucket_release_position = 0.5;

        try
        {
            v_servo_bucket = hardwareMap.servo.get ("bucket");
            v_servo_bucket.setPosition (initial_bucket_position);
        }
        catch (Exception p_exeception)
        {
        ...
        try
        {
            v_servo_bucket_release = hardwareMap.servo.get ("bucket_release");
            v_servo_bucket_release.setPosition (initial_bucket_release_position);
        }
        catch (Exception p_exeception)
        {
            m_warning_message ("bucket_release");
            DbgLog.msg (p_exeception.getLocalizedMessage ());

            v_servo_bucket_release = null;
        }
...

    {
        if (v_motor_left_arm != null)
        {
            v_motor_left_arm.setPower (p_left_power);
        }
        if (v_motor_right_arm != null)
        {
            v_motor_right_arm.setPower (p_right_power);
        }
...

    {
        ...
        if (v_motor_right_arm != null)
        {
            ...
            // TODO Implement stall code using these variables.
            //
            if (Math.abs (v_motor_right_arm.getCurrentPosition ()) > p_count)
            {
                //
                // Set the status to a positive indication.
                //
     ...
     */
    double a_bucket_position ()
    {
        double l_return = 0.0;

        if (v_servo_bucket != null)
        {
            l_return = v_servo_bucket.getPosition ();
        }
    ...
    //--------------------------------------------------------------------------
    //
    // a_bucket_release_position
    //
    /**
     * Access the bucket release position.
     */
    double a_bucket_release_position ()
    {
        double l_return = 0.0;

        if (v_servo_bucket_release != null)
        {
            l_return = v_servo_bucket_release.getPosition ();
        }
    ...
    //
    /**
     * Change the bucket position.
     */

    void m_bucket_position (double p_position)
    {
        //
        ...
        if (v_servo_bucket != null)
        {
            v_servo_bucket.setPosition (l_position);
        }
    ...
    void m_bucket_release_position (double p_position)
    {
        //
        // Ensure the specific value is legal.
        //
        ...
        if (v_servo_bucket_release != null)
        {
            v_servo_bucket_release.setPosition (l_position);
        }

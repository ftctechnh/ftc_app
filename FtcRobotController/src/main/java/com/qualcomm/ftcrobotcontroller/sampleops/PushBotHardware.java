...
public class PushBotHardware extends OpMode {
...

	{
		...
		//
		this.v_dc_motor_controller_drive = this.hardwareMap.dcMotorController.get("drive_controller");

		this.v_motor_left_drive = this.hardwareMap.dcMotor.get("left_drive");

		this.v_motor_right_drive = this.hardwareMap.dcMotor.get("right_drive");
		this.v_motor_right_drive.setDirection(DcMotor.Direction.REVERSE);

		//
		// Connect the arm motor.
		//
		this.v_motor_left_arm = this.hardwareMap.dcMotor.get("left_arm");
		...
		this.v_servo_left_hand = this.hardwareMap.servo.get("left_hand");
		this.v_servo_left_hand.setPosition(l_hand_position);

		this.v_servo_right_hand = this.hardwareMap.servo.get("right_hand");
	...
	double scale_motor_power(double p_power) {
		...
		double[] l_array = {0.00, 0.05, 0.09, 0.10, 0.12, 0.15, 0.18, 0.24, 0.30, 0.36, 0.43, 0.50, 0.60,
				0.72, 0.85, 1.00, 1.00};
	...
	double a_left_drive_power() {
		return this.v_motor_left_drive.getPower();
	...
	double a_right_drive_power() {
		return this.v_motor_right_drive.getPower();
...

	{
		DcMotorController.RunMode l_mode =
				this.v_dc_motor_controller_drive.getMotorChannelMode(this.v_channel_left_drive);
		if (l_mode == DcMotorController.RunMode.RESET_ENCODERS) {
			this.v_dc_motor_controller_drive.setMotorChannelMode(this.v_channel_left_drive,
					DcMotorController.RunMode.RUN_USING_ENCODERS);
		}

		l_mode = this.v_dc_motor_controller_drive.getMotorChannelMode(this.v_channel_right_drive);
		if (l_mode == DcMotorController.RunMode.RESET_ENCODERS) {
			this.v_dc_motor_controller_drive.setMotorChannelMode(this.v_channel_right_drive,
					DcMotorController.RunMode.RUN_USING_ENCODERS);
		}
	...
	int a_left_encoder_count() {
		return this.v_motor_left_drive.getCurrentPosition();
	...
	int a_right_encoder_count() {
		return this.v_motor_right_drive.getCurrentPosition();
	...
	boolean have_drive_encoders_reached(double p_left_count, double p_right_count) {
		...
		//
		if ((Math.abs(this.v_motor_left_drive.getCurrentPosition()) > p_left_count)
				&& (Math.abs(this.v_motor_right_drive.getCurrentPosition()) > p_right_count)) {
			//
	...
	boolean have_drive_encoders_reset() {
		...
		//
		if ((this.a_left_encoder_count() == 0) && (this.a_right_encoder_count() == 0)) {
			//
			// Set the status to a positive indication.
			//
			l_status = true;
	...
	double a_left_arm_power() {
		return this.v_motor_left_arm.getPower();
	...
	double a_hand_position() {
		return this.v_servo_left_hand.getPosition();
	...
	void m_hand_position(double p_position) {
		...
		this.v_servo_right_hand.setPosition(1.0 - l_position);

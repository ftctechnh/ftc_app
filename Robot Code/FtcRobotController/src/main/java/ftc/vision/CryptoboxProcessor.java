...
public class CryptoboxProcessor implements ImageProcessor<CryptoBoxResult> {
    private static final String TAG = "CryptoboxProcessor";
    private static final double MIN_MASS = 4;
    ...
    public ImageProcessorResult<CryptoBoxResult> process(long startTime, Mat rgbaFrame, boolean saveImages) {
        ...
        // The HSV thresholds for each color (red, white, blue) are stored as a list of min HSV
        ...
        hsvMin.add(new Scalar( 0,  0, 205)); //white min
        hsvMax.add(new Scalar(255, 51, 255)); //white max
        ...
        // with  0=red  1=white  2=blue  3=UNKNOWN
        // So both sides start as unknown:
        int[] maxMassIndex = {3, 3, 3}; // index of the max mass
        ...
        CryptoBoxResult.CryptoBoxColor[] BallColours = CryptoBoxResult.CryptoBoxColor.values();
        CryptoBoxResult.CryptoBoxColor left = BallColours[maxMassIndex[0]];
        CryptoBoxResult.CryptoBoxColor right = BallColours[maxMassIndex[1]];
        ...
        return new ImageProcessorResult<>(startTime, rgbaFrame,
                new CryptoBoxResult(left, right)
        );
    }
}